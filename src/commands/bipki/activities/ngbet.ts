...
function distributePoolRain(
  ...
  return randomlyDistribute(others, state.total, Math.min(5, others.length))
}
...
Общий банк: ${bold(String(state.total))} ${pluralizeBipki(state.total)}`)
      ...
      const { amount, error } = parsePositiveAmount(args)
      if (error) {
      ...
      try {
        ...
        await ctx.reply(format`${bold('🎰 NaganBet')}
${userName(ctx.from, userId)} поставил ${bold(String(amount))} ${pluralizeBipki(amount)}
Общий банк: ${bold(String(state.total))} ${pluralizeBipki(state.total)}`)
      } catch (e: any) {
        const msg =
