...
async function searchWOBooru(tags, ctx) {
  ...
  try {
    ...
    if (data["@attributes"].count == 0) {
      ctx.sendMessage("Ничего не нашлось");
      return false;
    }
    ...
    const photos = data.post.map((e) => {
      return {
        type: "photo",
        media: e.file_url,
        has_spoiler: e.rating === "explicit",
        caption: `score: ${e.score} / id: ${link(
          e.id,
          "https://gelbooru.com/index.php?page=post&s=view&id=" + e.id
        )}`,
      };
    });
    ...
    try {
      ...
      setTimeout(() => {
        searchWOBooru(tags, ctx);
      }, 3000);
    }
  } catch (e) {
    ctx.sendMessage(`Ошибка: ${e.message}`);
  }
}
...
const searchCommand = (tags, ctx, site = "gb", t = 0) => {
  ...
  Booru.search(site, tags, { limit: 3, random: true }).then((res) => {
    if (!res.posts.length) {
      ctx.sendMessage("Ничего не найдено");
      return false;
    }
    ...
    const photos = res.posts.map((e) => {
      return {
        ...
        caption: `score: ${e.score} / id: ${link(
          e.id,
          e.booru.domain + e.booru.site.api.postView + e.id
        )}`,
        parse_mode: "Markdown",
      };
    });
    ...
    ctx
      ...
      .catch((err) => {
        if (t < 3)
          setTimeout(() => {
            searchCommand(tags, ctx, site, t + 1);
          }, 3000);
