...
function loadPools(): void {
  const rows = bipbank.restorePools()
...
Общий банк: ${bold(String(state.total))} ${pluralizeBipki(state.total)}`)
      ...
      if (args.toLowerCase() === 'cancel') {
        ...
        bipbank.poolRefund(chatId, 'ngbet')
      ...
      try {
        bipbank.poolContribute(chatId, 'ngbet', userId, amount)
...
${join(items, '\n')}`
      ...
      if (killedUserId) {
        const total = bipbank.poolAward(chatId, 'ngbet', killedUserId)
        states.delete(chatId)
        await ctx.reply(format`${bold('🎰 NaganBet')}
